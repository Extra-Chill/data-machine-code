...
namespace {
	...
	$repo->runs['cleanup-run-active-safe'] = array(
		'run_id'  => 'cleanup-run-active-safe',
		'mode'    => 'safe_workspace_cleanup',
		'status'  => 'applying',
		'summary' => array(
			'safe_cleanup_progress' => array(
	...
	$repo->runs['cleanup-run-queued-safe'] = array(
		'run_id'  => 'cleanup-run-queued-safe',
		'mode'    => 'safe_workspace_cleanup',
		'status'  => 'queued',
		'summary' => array(
			'safe_cleanup_progress' => array(
	...
	$command->cleanup(
		...
		array(
			'format'     => 'json',
			'request-id' => 'disconnected-client-1',
		)
	);
	...
	$repo->runs['cleanup-run-disconnected-safe'] = array(
		...
		'policy'     => array(
			'dry_run'    => true,
			'source'     => DataMachineCode\Cli\Commands\WorkspaceCommand::CLEANUP_CLI_SOURCE,
			'request_id' => 'disconnected-client-1',
		),
	);
