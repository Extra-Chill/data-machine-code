...
namespace {
	...
	final class TestWorkspaceSafeCleanupTask extends \DataMachineCode\Tasks\WorkspaceSafeCleanupTask {
		...
		public function __construct( private SafeCleanupTaskRepository $repository ) {
			$this->next_result = array(
				'success' => true,
				'run_id'  => 'cleanup-run-safe-task',
	...
	workspace_safe_schedule_assert(
		array(
			'source'       => 'recurring_schedule',
			'limit'        => 25,
			'passes'       => 5,
			'cycles'       => 5,
			'until_budget' => '45s',
		) === ( $schedule['task_params'] ?? null ),
		'maintenance schedule declares bounded invocation parameters'
	);
