...
class WorkspaceCommand extends BaseCommand {
	...
	private const WORKTREE_OPERATIONS = array(
		...
		'abandoned'                               => array( 'ability' => 'datamachine-code/workspace-worktree-abandoned-cleanup' ),
	...
	/**
	 * WP-CLI callback for a scoped worktree operation.
	 *
	 ...
	 *   worktree (applies to `add` only). Default behavior projects the site's
	 *   context through installed runtime integrations and adds injected paths to
	 *   the repository's `info/exclude`. The ability-level input is
	 *   `inject_context=false`; this flag is the CLI shorthand.
	 *
	 * [--skip-bootstrap]
	 ...
	 * @subcommand worktree
	 */
	private function worktree( array $args, array $assoc_args ): void {
		$operation = $args[0] ?? '';

		if ( '' === $operation ) {
			WP_CLI::error('Usage: wp datamachine-code workspace worktree <add|get|list|remove|prune|locks|handoff-revalidate|cleanup|cleanup-artifacts|abandoned|bounded-cleanup-eligible-apply|cleanup-eligible-drain|emergency-cleanup|reconcile-metadata|capacity-recovery|backfill-origin-session|active-no-signal-report|active-no-signal-finalized-apply|active-no-signal-equivalent-clean-apply|active-no-signal-merged-apply|active-no-signal-remote-clean-apply|active-no-signal-drain|refresh-context|finalize|mark-cleanup-eligible> [<repo>] [<branch>] [--flags]');
			return;
		}
