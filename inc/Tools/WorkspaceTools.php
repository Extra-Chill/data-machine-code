...
class WorkspaceTools extends BaseTool
{
    ...
    public function getWorktreeAddDefinition(): array
    {
        return $this->progressDefinition(
            ...
            'reuse_policy'   => array( 'type' => 'string', 'enum' => array( 'reuse_compatible', 'isolated', 'recycle_terminal' ), 'description' => 'Reuse and allocation behavior. reuse_compatible reuses only an exact compatible handle and refuses another same-repo handle for the same task. isolated requires purpose, owner_run_ref, and cleanup_policy=remove_on_success for parallel same-task work. recycle_terminal resets only a proven-clean terminal exact handle whose HEAD is contained in the requested base.' ),
