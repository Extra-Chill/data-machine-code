...
final class WorkspaceLockStore {
	...
	public static function heartbeat( int $lock_id, array $metadata = array() ): bool|\WP_Error {
		...
		$existing = $wpdb->get_var($wpdb->prepare("SELECT metadata_json FROM {$table} WHERE id = %d AND status = %s", $lock_id, 'active')); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Internal table name is generated by self::table_name().
		$metadata = array_merge(self::decode_metadata((string) $existing), $metadata);
		$time     = self::now_timestamp();
		...
		$updated = \DataMachineCode\Storage\SqliteBusyRetry::run(
			...
			static fn() => $wpdb->update(
				...
				array( 'id' => $lock_id, 'status' => 'active' ),
				array( '%s', '%s', '%s' ),
				array( '%d', '%s' )
			)
		);
