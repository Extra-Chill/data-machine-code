...
class Workspace {
	 ...
	 */
	public function diff_summary( string $name, ?string $from = null, ?string $to = null, bool $staged = false, ?string $path = null ): array|\WP_Error {
		$repo_path = $this->resolve_repo_path( $name );
		if ( is_wp_error( $repo_path ) ) {
			return $repo_path;
		}
		...
		if ( null !== $path && '' !== trim( $path ) ) {
			...
			$path_args = array( '--', escapeshellarg( $relative ) );
		}

		$base_args = $this->build_diff_args( $from, $to, $staged );
		$numstat   = $this->run_git( $repo_path, implode( ' ', array_merge( $base_args, array( '--numstat' ), $path_args ) ) );
		if ( is_wp_error( $numstat ) ) {
			return $numstat;
		}

		$name_status = $this->run_git( $repo_path, implode( ' ', array_merge( $base_args, array( '--name-status' ), $path_args ) ) );
		if ( is_wp_error( $name_status ) ) {
			return $name_status;
		}

		$shortstat = $this->run_git( $repo_path, implode( ' ', array_merge( $base_args, array( '--shortstat' ), $path_args ) ) );
		if ( is_wp_error( $shortstat ) ) {
			return $shortstat;
		}

		$status = $this->run_git( $repo_path, 'status --porcelain' . ( empty( $path_args ) ? '' : ' -- ' . escapeshellarg( trim( (string) $path ) ) ) );
		if ( is_wp_error( $status ) ) {
			return $status;
		}
	 ...
	 * @return array<string,mixed>|\WP_Error
	 */
	public function diff_validate( string $name, array $config ): array|\WP_Error {
		$summary = $this->diff_summary(
			$name,
	...
	/**
	 * Build common git diff arguments for summary/diff reads.
	 *
	 * @param string|null $from   Optional from ref.
	 * @param string|null $to     Optional to ref.
	 * @param bool        $staged Whether to diff staged changes.
	...
	/**
	 * Parse git numstat/name-status output into changed file records.
	 *
	 * @param string $numstat_output     Git --numstat output.
	 * @param string $name_status_output Git --name-status output.
	 * @return array<int,array<string,mixed>>
	 */
	...
	/**
	 * Add untracked porcelain entries to diff file metadata.
	 *
	 * @param array<int,array<string,mixed>> $files             Parsed diff files.
	 * @param string                         $porcelain_output Git status porcelain output.
	 * @return array<int,array<string,mixed>>
	 */
	...
	/**
	 * Determine whether a changed path looks like test coverage.
	 *
	...
	/**
	 * Normalize path pattern lists from tool input.
	 *
	...
	/**
	 * Return changed paths matching at least one pattern.
	 *
	 * @param string[] $paths    Changed paths.
	...
	/**
	 * Check whether a path matches any simple glob/prefix pattern.
	 *
	 * @param string   $path     Relative path.
	 * @param string[] $patterns Path patterns.
	 * @return bool
