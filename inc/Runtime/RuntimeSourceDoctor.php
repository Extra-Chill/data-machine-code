...
final class RuntimeSourceDoctor {
	...
	public static function apply( string $runtime_file, array $config = array() ): array|\WP_Error {
		...
		if ( realpath($source) === realpath($target) ) {
			return array(
				'success' => true,
				'changed' => false,
				'message' => 'Runtime already resolves to the authoritative source.',
			);
		}
		if ( is_link($target) ) {
		...
		try {
			...
			return array(
				...
				'state'   => 'failed',
				'changed' => false,
				'message' => 'External runtime reconciliation failed: ' . $error->getMessage(),
				'action'  => $reconciler['action'],
				'owner'   => $reconciler['owner'],
			);
		}
		if ( ! is_array($result) || true !== ( $result['success'] ?? false ) ) {
			return array(
				...
				'state'   => 'failed',
				'changed' => false,
				'message' => is_array($result) ? (string) ( $result['message'] ?? 'External runtime reconciler reported failure.' ) : 'External runtime reconciler returned an invalid result.',
				'action'  => $reconciler['action'],
				'owner'   => $reconciler['owner'],
			);
		}
	...
	private static function verifiedApplyResult( string $runtime_file, array $config, bool $changed, string $message, array $reconciler = array() ): array {
		...
		if ( 'aligned' !== ( $after['drift']['classification'] ?? '' ) ) {
			return array(
				...
				'changed'      => $changed,
				'message'      => 'Runtime reconciliation completed but active runtime identity did not verify.',
				'verification' => $after,
				'action'       => $reconciler['action'] ?? null,
				'owner'        => $reconciler['owner'] ?? null,
			);
		}
		return array(
			'success'      => true,
	...
	private static function externalReconciler( array $config ): array {
		...
		if ( is_callable($action['authorize_callback'] ?? null) ) {
			try {
				$authorized = true === call_user_func($action['authorize_callback']);
			} catch ( \Throwable ) {
				$authorized = false;
			}
		...
		if ( 'command' !== ( $action['type'] ?? '' ) || '' === trim( (string) ( $action['command'] ?? '' )) || ! $authorized ) {
			$action                  = array(
				'type'    => 'handoff',
				'code'    => 'runtime_external_reconciliation_required',
				'message' => 'Use the runtime owner\'s deployment integration to reconcile this copied or Git runtime.',
			);
			$reconciler['reconcile'] = null;
		}
		return array(
			'owner'      => trim( (string) ( $reconciler['owner'] ?? 'runtime owner' )),
			'action'     => $action,
			'reconcile'  => $reconciler['reconcile'] ?? null,
	...
	public static function classify( array $runtime, array $source, array $contract ): array {
		if ( ! empty($contract['drift']) ) {
			return array(
				'classification' => 'command_contract_drift',
				'reason'         => 'The source advertises a command flag that the active WP-CLI runtime does not register.',
			);
		}
		if ( empty($source['available']) ) {
			return array(
				'classification' => 'source_unavailable',
				'reason'         => (string) ( $source['reason'] ?? 'authoritative source is unavailable' ),
			);
		}
		if ( self::isVersion($runtime['version'] ?? '') && self::isVersion($source['version'] ?? '') ) {
			...
			if ( $comparison < 0 ) {
				return array(
					'classification' => 'runtime_predates_source',
					'reason'         => 'The installed runtime version predates the authoritative source version.',
				);
			}
		}
		if ( ( $runtime['head'] ?? '' ) !== '' && ( $runtime['head'] ?? '' ) === ( $source['head'] ?? '' ) ) {
			return array(
				...
				'reason'         => 'Runtime and authoritative source resolve to the same Git head.',
			);
		}
		if ( 'verified' === ( $runtime['package_provenance']['state'] ?? '' ) ) {
			return array(
				'classification' => 'aligned',
				'reason'         => 'Runtime package provenance matches the configured release source.',
			);
		}
		if ( ( $runtime['fingerprint'] ?? '' ) !== '' && ( $runtime['fingerprint'] ?? '' ) === ( $source['fingerprint'] ?? '' ) ) {
			return array(
				'classification' => 'aligned',
				'reason'         => 'Runtime and authoritative source have identical file content.',
			);
		}
		if ( 'git_checkout' === ( $runtime['deployment'] ?? '' ) && 'git_checkout' === ( $source['deployment'] ?? '' ) ) {
			...
			if ( in_array($relation, array( 'behind', 'ahead', 'diverged' ), true) ) {
				return array(
					'classification' => 'runtime_' . $relation . '_source',
					'reason'         => 'Git ancestry comparison identified runtime/source drift.',
				);
			}
		}
		return array(
			'classification' => 'runtime_source_drift',
			'reason'         => 'Runtime and authoritative source identities differ.',
		);
	}
	...
	private static function identity( string $path, string $version, bool $include_fingerprint = true ): array {
		...
		$real       = false !== $real_path ? $real_path : $path;
		...
		return array(
			...
			'branch'          => $branch,
			'head'            => $head,
			'fingerprint'     => $include_fingerprint ? self::fingerprint($real) : '',
			'package_headers' => self::packageHeaders($real),
		);
	}
	...
	private static function sourceIdentity( string $path, bool $include_fingerprint = true ): array {
		if ( '' === $path ) {
			return array(
				'available' => false,
				'reason'    => 'source_path_not_configured',
			);
		}
		if ( ! self::isSource($path) ) {
			return array(
				'available' => false,
				'path'      => $path,
				'reason'    => 'source_entrypoint_missing',
			);
		}
	...
	private static function releaseIdentity( string $source_path, string $release_ref ): array {
		if ( '' === $source_path || ! is_dir($source_path) || '' === self::git($source_path, 'rev-parse --git-dir') ) {
			return array(
				'available' => false,
				'ref'       => $release_ref,
				'reason'    => 'source_git_checkout_unavailable',
			);
		}
		...
		if ( '' !== $tag_head ) {
			...
			return array(
				...
				'kind'         => 'immutable_tag',
				'head'         => $tag_head,
				'version'      => self::versionFromBody($body),
			);
		}

		$branch     = (string) preg_replace('#^origin/#', '', $release_ref);
		...
		if ( '' === $head ) {
			return array(
				...
				'reason'       => '' === $local ? 'remote_tracking_ref_unavailable' : 'local_ref_not_accepted',
				'local_ref'    => $local,
			);
		}
		...
		return array(
			...
			'version'         => self::versionFromBody($body),
			'local_ref'       => $local,
			'local_ref_state' => self::localRefState($source_path, $local, $head),
		);
	}
	...
	private static function localRefState( string $path, string $local, string $remote ): string {
		...
		if ( self::git($path, 'merge-base ' . escapeshellarg($local) . ' ' . escapeshellarg($remote)) === $local ) {
			return 'stale';
		}
		if ( self::git($path, 'merge-base ' . escapeshellarg($local) . ' ' . escapeshellarg($remote)) === $remote ) {
			return 'ahead';
		}
		return 'diverged';
	}
	...
	private static function packageHeaders( string $path ): array {
		$body    = is_readable($path . '/data-machine-code.php') ? (string) file_get_contents($path . '/data-machine-code.php') : ''; // phpcs:ignore WordPress.WP.AlternativeFunctions.file_get_contents_file_get_contents -- Package headers are local metadata.
		$headers = array();
		foreach ( array( 'Package-Version', 'Package-Source-Tag', 'Package-Source-Commit', 'Package-Digest' ) as $name ) {
			if ( preg_match('/^\s*\*\s*' . preg_quote($name, '/') . ':\s*(.+)$/mi', $body, $matches) ) {
	...
	private static function packageProvenance( array $runtime, string $source_path, array $release, array $trusted ): array {
		...
		foreach ( $required as $field ) {
			if ( '' === trim( (string) ( $headers[ $field ] ?? '' )) ) {
				return array( 'state' => 'not_present' );
			}
		}
		if ( ! self::isTrustedProvenance($trusted) ) {
			return array(
				'state'  => 'untrusted',
				'reason' => 'trusted_release_provenance_unavailable',
			);
		}
		...
		if ( ! hash_equals( (string) $trusted['package_digest'], $digest) || ! hash_equals( (string) $trusted['package_digest'], (string) $headers['Package-Digest']) ) {
			return array(
				'state'  => 'invalid',
				'reason' => 'package_digest_mismatch',
			);
		}
		if ( empty($release['available']) || ( $release['head'] ?? '' ) !== $headers['Package-Source-Commit'] || $headers['Package-Source-Commit'] !== $tag || ( $runtime['version'] ?? '' ) !== $headers['Package-Version'] || ( $release['version'] ?? '' ) !== $headers['Package-Version'] || $trusted['version'] !== $headers['Package-Version'] || $trusted['source_tag'] !== $headers['Package-Source-Tag'] || $trusted['source_commit'] !== $headers['Package-Source-Commit'] ) {
			return array(
				'state'  => 'invalid',
				'reason' => 'package_source_mismatch',
			);
		}
		return array(
			'state'          => 'verified',
			'version'        => $headers['Package-Version'],
			'source_tag'     => $headers['Package-Source-Tag'],
			'source_commit'  => $headers['Package-Source-Commit'],
			'package_digest' => $headers['Package-Digest'],
		);
	}

	/** @return array<string,mixed> */
	private static function trustedProvenanceIdentity( array $provenance ): array {
		return self::isTrustedProvenance($provenance) ? array(
			'state'          => 'configured',
	...
	public static function injectPackageProvenance( string $package_path, string $version, string $source_tag, string $source_commit ): array|\WP_Error {
		...
		$body = (string) file_get_contents($file); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_get_contents_file_get_contents -- Package provenance is injected into a local file.
		foreach ( array(
			'Package-Version'       => $version,
			'Package-Source-Tag'    => $source_tag,
			'Package-Source-Commit' => $source_commit,
			'Package-Digest'        => '',
		) as $name => $value ) {
			$line = ' * ' . $name . ': ' . $value;
			if ( preg_match('/^\s*\*\s*' . preg_quote($name, '/') . ':.*$/mi', $body) ) {
				$body = preg_replace('/^\s*\*\s*' . preg_quote($name, '/') . ':.*$/mi', $line, $body) ?? $body;
			} else {
				$body = preg_replace('/\*\//', $line . "\n */", $body, 1) ?? $body;
			}
		}
		if ( false === file_put_contents($file, $body) ) { // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_file_put_contents -- Package provenance is injected into a local file.
			return new \WP_Error('package_provenance_write_failed', 'Could not write package provenance headers.');
		}
		$digest = self::fingerprint($package_path, true);
		$body   = (string) file_get_contents($file); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_get_contents_file_get_contents -- Package provenance is injected into a local file.
		$body   = preg_replace('/^(\s*\*\s*Package-Digest:).*$/mi', '$1 ' . $digest, $body) ?? $body;
		if ( false === file_put_contents($file, $body) ) { // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_file_put_contents -- Package provenance is injected into a local file.
			return new \WP_Error('package_provenance_write_failed', 'Could not write package provenance digest.');
		}
		return array(
			'version'        => $version,
	...
	private static function contract( string $source_path, array $contract ): array {
		...
		$source_supports  = is_dir($source_path) && false !== strpos( (string) @shell_exec('grep -R -- ' . escapeshellarg($flag) . ' ' . escapeshellarg($source_path) . ' 2>/dev/null'), $flag); // phpcs:ignore WordPress.PHP.DiscouragedPHPFunctions.system_calls_shell_exec,WordPress.PHP.NoSilencedErrors.Discouraged -- A failed read means the inspected flag is absent.
		$runtime_supports = isset($contract['runtime_supports']) ? (bool) $contract['runtime_supports'] : null;
		return array(
			'checked'          => null !== $runtime_supports,
			'flag'             => $flag,
			'source_supports'  => $source_supports,
			'runtime_supports' => $runtime_supports,
			'drift'            => $source_supports && false === $runtime_supports,
		);
	}

	private static function pluginVersion( string $path ): string {
		$file = $path . '/data-machine-code.php';
		$body = is_readable($file) ? (string) file_get_contents($file) : ''; // phpcs:ignore WordPress.WP.AlternativeFunctions.file_get_contents_file_get_contents -- Plugin version is local metadata.
	...
	private static function fingerprint( string $path, bool $package_digest = false ): string {
		...
		foreach ( $paths as $relative ) {
			$contents = file_get_contents($path . '/' . $relative); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_get_contents_file_get_contents -- Fingerprints require exact local file bytes.
			if ( false === $contents ) {
				return '';
			}
	...
	private static function git( string $path, string $args ): string {
		return trim( (string) @shell_exec('git -C ' . escapeshellarg($path) . ' ' . $args . ' 2>/dev/null')); // phpcs:ignore WordPress.PHP.DiscouragedPHPFunctions.system_calls_shell_exec,WordPress.PHP.NoSilencedErrors.Discouraged -- Git failure is represented by an empty identity.
	}
	...
	private static function gitRelation( string $runtime, string $source ): string {
		...
		if ( $source_head === $runtime_head ) {
			return 'aligned';
		}
		if ( self::git($source, 'merge-base HEAD ' . escapeshellarg($runtime_head)) === $runtime_head ) {
			return 'behind';
		}
		if ( self::git($runtime, 'merge-base HEAD ' . escapeshellarg($source_head)) === $source_head ) {
			return 'ahead';
		}
		return 'diverged';
	}
