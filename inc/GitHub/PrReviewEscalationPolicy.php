...
class PrReviewEscalationPolicy {
	...
	private static function classifyPath( string $path ): array {
		...
		if ( preg_match('#(^|/)(auth|permissions?|secrets?|tokens?|webhooks?)(/|[-_.]|$)#', $path_lower)
		...
		if ( preg_match('#(^|/)(migrations?|schema|jobs?|queues?|action-scheduler|actionscheduler)(/|[-_.]|$)#', $path_lower)
		...
		if ( preg_match('#(^|/)(inc/abilities|inc/tools|inc/cli|inc/rest|rest|cli|abilities?|tools?)(/|[-_.]|$)#', $path_lower)
		) {
			$groups['public_api'] = 'touches_public_api_surface';
		}

		if ( preg_match('#(^|/)(readme|docs?|documentation|examples?|assets|templates?)(/|[-_.]|$)#', $path_lower)
